    ...
    <div className="p-4 md:p-8 max-w-[1600px] mx-auto h-[calc(100vh-64px)] flex flex-col">
                        ...
                        <div className="flex justify-between items-start mb-3">
                          <div>
                            <span className="inline-flex items-center gap-1.5 px-2 py-1 rounded-md bg-slate-100 text-slate-600 text-xs font-semibold mb-2">
                              {order.table ? <UtensilsCrossed className="w-3 h-3" /> : <ShoppingBag className="w-3 h-3" />}
                              #{order.id}
                            </span>
                            <h4 className="font-bold text-slate-800 text-lg leading-tight">
                              {order.table?.number || order.customerName || "Takeaway"}
                            </h4>
                          </div>
      ...
      {selectedOrder && (
        <OrderDetailModal
          ...
          onUpdateStatus={(id, status) => handleUpdateStatus(Number(id), status)}
          onProcessPayment={() => {}}
        />
      )}
    </div>
